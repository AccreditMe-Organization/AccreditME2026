...
Tech Stack → Frontend) — built after Section 10.5's own `onWheel()`
boundary-guard (ACC-36/38) was found to be logically incapable of the
failure mode it was written for: a short option list that fits
entirely within its own listbox has `scrollTop` permanently `0` with
`atBottom` permanently `true`, so the boundary check is satisfied on
literally every wheel tick — zero real protection, not partial
protection, confirmed via reproducing `position-form`'s Mapped Role
picker (5 options) failing inside `EditDialogComponent` after ACC-38
had already marked three OTHER pickers (`calendar-config`,
`email-provider-settings`, `user-profile`'s `actingOrgUnitId` — all
routed-page/`<main>`-level, not dialog-level) as hand-tested PASS.

**Confirmed root cause** (verified against PrimeNG's own source,
`primeng-dom.mjs`'s `ConnectedOverlayScrollHandler`): it calls
`DomHandler.getScrollableParents()` to find every real scrollable
ancestor of the trigger and binds a direct `scroll` listener to each
one that closes the overlay unconditionally — not a bug fixable by
configuration, a structural property of how PrimeNG's connected
overlays are built. No `onWheel()`-style boundary guard can compensate
for this category, since the guard fires on the *listbox's own*
scroll boundary while the actual close is triggered by the
*ancestor's* scroll, a different element entirely.

**The fix, and why it's structurally different, not just
better-tuned**: built on Angular CDK's `Overlay` +
`RepositionScrollStrategy` (`@angular/cdk/overlay`) instead of
PrimeNG's connected-overlay system. Verified directly against CDK's
own source (`_overlay-module-chunk.mjs`): on ancestor scroll it calls
`updatePosition()` (repositions to track the trigger) and only ever
calls `detach()` if `autoClose: true` is explicitly configured *and*
the overlay has scrolled fully outside the viewport — never on
ancestor scroll alone. `OverlaySelectComponent` configures
`{ autoClose: true, scrollThrottle: 20 }`.

**A second, independently-confirmed gap this surfaced**: CDK's own
`ScrollDispatcher` (which `RepositionScrollStrategy` subscribes to)
only reacts to a single bubble-phase `document`-level `scroll`
listener plus whatever containers are explicitly registered via
`[cdkScrollable]` — and nested `overflow: auto` elements' native
`scroll` events do not bubble to `document` at all (standard DOM
behavior, unlike PrimeNG's direct-listener-per-ancestor approach).
Nothing in this codebase uses `[cdkScrollable]` anywhere, so without
addressing this, `RepositionScrollStrategy` would never learn a
dialog's own scroll area scrolled — neither repositioning nor closing.
`[cdkScrollable]` as a template directive is not reachable for
PrimeNG-internal markup either (`.p-dialog-content` is generated by
`p-dialog`'s own template, not this app's — Angular directive matching
is static, resolved against the template that declares the element,
never against runtime-added classes). Resolved with an imperative,
self-contained mechanism inside `OverlaySelectComponent` itself: on
`open()`, it walks up from its own trigger (`findScrollableAncestors()`,
mirroring PrimeNG's own `getScrollableParents()` algorithm exactly —
same computed-style `overflow`/`overflow-x`/`overflow-y` check) and
registers a minimal object with `ScrollDispatcher.register()` for each
real ancestor found, verified against `scrolling.mjs` to need only
`.elementScrolled()` — deregistered on every detach path uniformly.
Works identically for `EditDialogComponent`'s own scroll area and
PrimeNG's internal `.p-dialog-content`, without touching either file.

**OverlayRef lifecycle** — created once, lazily, on first `open()`,
and reused via `attach()`/`detach()` for every subsequent cycle;
`dispose()` only in `ngOnDestroy()`. Verified directly against CDK's
own `CdkMenuTrigger` (`@angular/cdk/menu`, the same "persistent
trigger, repeatedly opened/closed" shape): `this.overlayRef =
this.overlayRef || createOverlayRef(...)` in `open()`, plain
`.detach()` in `close()`, `.dispose()` called solely from
`_destroyOverlay()`, itself called only from `ngOnDestroy()`. An
earlier version of this component created a fresh `OverlayRef` on
every `open()`, which left each cycle's now-empty host/pane DOM nodes
behind in `.cdk-overlay-container` forever (`detach()` alone never
removes them) — caught and fixed before this component was finalized,
verified via live measurement (Playwright against the running app:
`.cdk-overlay-container`'s child count confirmed flat, not cumulative,
across 5 real open/close cycles) and via a committed regression test.

**Keyboard nav/ARIA come from `@angular/cdk/listbox`
(`CdkListbox`/`CdkOption`), not hand-written key handlers** — verified
against its source: arrows/Home/End/typeahead all route through CDK's
own `ActiveDescendantKeyManager`; SPACE/ENTER share the same
`triggerOption()` path as a mouse click. It never touches Escape, so
that falls through untouched to the overlay's own `keydownEvents()`
handling.

**Escape closes only the dropdown, never a parent dialog** — CDK's
`OverlayKeyboardDispatcher` binds one `keydown` listener on `body`,
routed only to the topmost *attached* overlay; PrimeNG's own
`Dialog.bindDocumentEscapeListener()` (`primeng-dialog.mjs`) binds on
`document`, encountered later in the bubble chain than `body`.
`event.stopPropagation()` inside the CDK-routed handler — still
synchronous within that `body`-listener callback — stops the event
before it ever reaches `document`. Because the dispatcher only routes
to overlays present in `_attachedOverlays` (added on `attach()`,
removed on `detach()`), this interception exists exactly while the
dropdown is open; a parent dialog's own Escape-to-close is completely
unaffected once it's closed.

**Both `p-select` option-list shapes supported**: an object array with
`optionLabel`/`optionValue` property lookups (e.g. `position-form`'s
`RoleDto[]`), and a primitive array treated as its own label/value
when the lookup wouldn't resolve to anything meaningful (e.g.
